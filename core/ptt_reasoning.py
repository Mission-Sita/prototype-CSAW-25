...
class PTTReasoningModule:
    ...
    def __init__(self, tree_manager: TaskTreeManager):
        ...
        # Try to import prompt loader lazily
        try:
            from from_baseline.prompt_loader import load_category_prompts, render_template  # type: ignore
            self._load_category_prompts = load_category_prompts
            self._render_template = render_template
        except Exception:
            self._load_category_prompts = None
...
"""
        ...
        # If baseline prompts are available and a category is provided in constraints, prepend system/initial
        baseline_prefix = ""
        category = constraints.get('category') if isinstance(constraints, dict) else None
        if category and getattr(self, '_load_category_prompts', None):
            prompts = self._load_category_prompts(category)
            if prompts:
                context = {
                    'challenge': constraints.get('challenge', {}),
                    'environment': constraints.get('environment', {}),
                    'prompter': constraints.get('prompter', {}),
                }
                system = prompts.get('system', '')
                initial = prompts.get('initial', '')
                server_desc = ''
                server_type = (constraints.get('challenge', {}) or {}).get('server_type') if isinstance(constraints.get('challenge', {}), dict) else None
                if server_type == 'nc':
                    server_desc = prompts.get('nc_server_description', '')
                elif server_type == 'web':
                    server_desc = prompts.get('web_server_description', '')
                # Render templates if renderer exists
                if self._render_template:
                    system = self._render_template(system, context)
                    initial = self._render_template(initial, context)
                    server_desc = self._render_template(server_desc, context)
                baseline_prefix = f"{system}\n\n{initial}\n\n{server_desc}\n\n"

        prompt = f"""{baseline_prefix}You are an autonomous security agent initializing a Pentesting Task Tree (PTT) for a security assessment.
...
"""
        ...
        # If baseline prompts specify a "continue" line, prepend it
        baseline_continue = ""
        constraints = getattr(self.tree_manager, 'constraints', {}) if hasattr(self.tree_manager, 'constraints') else {}
        category = constraints.get('category') if isinstance(constraints, dict) else None
        if category and getattr(self, '_load_category_prompts', None):
            prompts = self._load_category_prompts(category)
            if prompts:
                cont = prompts.get('continue', '') or prompts.get('keep_going', '')
                if cont and self._render_template:
                    context = {
                        'challenge': constraints.get('challenge', {}),
                        'environment': constraints.get('environment', {}),
                        'prompter': constraints.get('prompter', {}),
                    }
                    cont = self._render_template(cont, context)
                baseline_continue = cont + "\n\n" if cont else ""

        prompt = f"""{baseline_continue}You are managing a Pentesting Task Tree (PTT) and need to select the next action.
